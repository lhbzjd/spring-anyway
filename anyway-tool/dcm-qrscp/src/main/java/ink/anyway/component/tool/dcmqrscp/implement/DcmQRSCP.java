...
package ink.anyway.component.tool.dcmqrscp.implement;
...
import ink.anyway.component.common.util.DicomUtil;
...
public class DcmQRSCP {
    ...
    private final class CStoreSCPImpl extends BasicCStoreSCP {
        ...
        CStoreSCPImpl(EventBus eventBus, ReceiveHandler receiveHandler) {
            super("*");
...
//                file = dest;
                ...
                DicomUtil.loadDicomInfo(info, attrs);

                info.setLocalFilePath(file.getAbsolutePath());
                ...
                if(info.getHaveHandle()==null||!info.getHaveHandle()){
                    LOG.warn(StringUtil.compose(Constants.DCMQRSCP_RECEIVE_LOG_PREFIX, JSON.toJSONString(info), " [without handle...]"));
                    throw new DicomServiceException(Status.ProcessingFailure, new Exception("without handler"));
                }

                if (addDicomDirRecords(as, attrs, fmi, file)) {
                    LOG.debug("{}: M-UPDATE {}", as, dicomDir);
                } else {
                    LOG.warn("{}: do not ignore received object", as);
//                    TODO deleteFile(as, file);
                }
...
//                TODO deleteFile(as, file);
    ...
    public DcmQRSCP(String[] params) throws IOException, ParseException {
        this(null, null, params);
    }

    public DcmQRSCP(EventBus eventBus, ReceiveHandler receiveHandler, String[] params) throws IOException, ParseException {
        this.eventBus = eventBus;
        this.receiveHandler = receiveHandler;

        this.cl = parseComandLine(params);
    ...
    private static void renameTo(Association as, File from, File dest)
            throws IOException {
        LOG.debug("{}: M-RENAME {}", new Object[]{ as, from, dest });
    ...
    private DicomServiceRegistry createServiceRegistry() {
        ...
        serviceRegistry.addDicomService(new CStoreSCPImpl(this.eventBus, this.receiveHandler));
    ...
    public void start() {
//        args = new String[]{"--ae-config", "D:\\thumbnail\\dcmqrscp\\ae.properties", "--all-storage", "-b", "DCMQRSCP@127.0.0.1:11112", "--dicomdir", "D:\\thumbnail\\dcmqrscp\\store\\record-file"};
        try {
    ...
    private static void configureDicomFileSet(DcmQRSCP main, CommandLine cl) throws Exception {
        if (!cl.hasOption("dicomdir")||!StringUtil.isValid(cl.getOptionValue("dicomdir")))
            throw new MissingOptionException(rb.getString("missing-dicomdir"));
        main.setDicomDirectory(new File(cl.getOptionValue("dicomdir").trim()));
    ...
    public Attributes calculateStorageCommitmentResult(String calledAET,
        ...
        try {
            ...
            LOG.warn("Failed to M-READ " + dicomDir, e);
            throw new DicomServiceException(Status.ProcessingFailure, e);
        }
